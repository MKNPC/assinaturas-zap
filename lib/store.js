...
async function loadGH() {
  ...
  if (!meta) return { data: { people: [] }, sha: null };

  let base64 = meta.content;
  if (!base64 && meta.sha) {
    base64 = await ghFetchBlob(meta.sha);
  }
  if (!base64) return { data: { people: [] }, sha: meta.sha };

  try {
    const db = JSON.parse(Buffer.from(base64, 'base64').toString('utf-8'));
    return { data: db, sha: meta.sha };
  } catch {
    return { data: { people: [] }, sha: meta.sha };
  }
}
...
async function saveToGithub(data) {
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const newSha = await saveGH(data, sha);
    if (typeof newSha === 'string') {
      ghCache = { data, sha: newSha };
      return true;
    }
