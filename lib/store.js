...
async function initRedis() {
  ...
  const url = process.env.KV_REST_API_URL || process.env.kv_rest_api_url || process.env.UPSTASH_REDIS_REST_URL;
  const token = process.env.KV_REST_API_TOKEN || process.env.kv_rest_api_token || process.env.UPSTASH_REDIS_REST_TOKEN;
  if (url && token) {
